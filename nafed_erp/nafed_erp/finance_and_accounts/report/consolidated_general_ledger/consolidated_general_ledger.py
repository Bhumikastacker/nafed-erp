...
def execute(filters=None):
    ...
    # -----------------------------
    # Convert MultiSelect company to list
    companies = filters.get("company")

    if isinstance(companies, str):
        # from MultiSelectList, it might be comma-separated string
        companies = [c.strip() for c in companies.split(",")]
    filters["company_list"] = companies
    # -----------------------------
	  # VERY IMPORTANT: ERPNext expects a SINGLE company string
    # So set company to first selected company
    if companies:
...
def set_account_currency(filters):
	if filters.get("account") or (filters.get("party") and len(filters.party) == 1):
		company_list = filters.get("company_list")
		filters["company_currency"] = frappe.get_cached_value("Company", company_list[0], "default_currency")
		...
		if filters.get("account"):
			...
			company_for_lookups = filters.get("company_list")[0] if filters.get("company_list") else filters.get("company")
			gle_currency = frappe.db.get_value(
				"GL Entry",
				{"party_type": filters.party_type, "party": filters.party[0], "company": company_for_lookups},
				"account_currency",
			)
...
def get_gl_entries(filters, accounting_dimensions):
	...
	if filters.get("include_default_book_entries"):
		company_for_lookups = filters.get("company_list")[0] if filters.get("company_list") else filters.get("company")
		filters["company_fb"] = frappe.get_cached_value("Company", company_for_lookups, "default_finance_book")
	...
	gl_entries = frappe.db.sql(
		f"""
		select
			name as gl_entry, posting_date , company, account, party_type, party,
			voucher_type, voucher_subtype, voucher_no, {dimension_fields}
			cost_center, project, {transaction_currency_fields}
			against_voucher_type, against_voucher, account_currency,
			against, is_opening, creation {select_fields}
		from `tabGL Entry`
		where company IN %(company_list)s {get_conditions(filters)}
		{order_by_statement}
	""",
		filters,
		as_dict=1,
...
def get_accountwise_gle(filters, accounting_dimensions, gl_entries, gle_map, totals):
	...
	if filters.get("show_net_values_in_party_account"):
		company_for_lookups = filters.get("company_list")[0] if filters.get("company_list") else filters.get("company")
		account_type_map = get_account_type_map(company_for_lookups)
...
def get_columns(filters):
	# determine a single company to use for company-specific lookups
	if filters.get("company_list"):
		company_for_lookups = filters.get("company_list")[0]
	else:
		company_for_lookups = filters.get("company") or get_default_company()

	if filters.get("presentation_currency"):
		currency = filters["presentation_currency"]
	else:
		filters["presentation_currency"] = currency = get_company_currency(company_for_lookups)

	company_currency = get_company_currency(company_for_lookups)
