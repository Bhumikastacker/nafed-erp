...
additional_salary = frappe.qb.DocType("Salary Deduction")
...
def get_earning_and_deduction_types(salary_slips):
	salary_component_and_type = {_("Earning"): [], _("Deduction"): [],_("Additional Deductions"): []}
...
def get_columns(earning_types, ded_types):
	...
	for deduction in ded_types:
		print("ssssssssssssssssssss", deduction)
		columns.append(
			{
				"label": deduction,
...
def get_salary_components(salary_slips):
	...
	employees = [d.employee for d in salary_slips]

	# Salary Detail Components
	salary_components = (
	...
	additional_components = frappe.db.sql("""
		SELECT DISTINCT deduction
		FROM `tabSalary Deduction`
""", as_dict=0)

	additional_components = [d[0] for d in additional_components]

	# Merge both
	return list(set(salary_components + additional_components))
...
def get_salary_slip_details(salary_slips, currency, company_currency, component_type):
	...
	additional_result = (
		...
		.select(
			additional_salary.parent,
			additional_salary.deduction,
			additional_salary.amount
		)
		.where(additional_salary.parent.isin(salary_slips))
	).run(as_dict=1)
	...
	# 🔥 ADD THIS BLOCK
	for d in additional_result:
		if not d.parent:
			continue

		ss_map.setdefault(d.parent, frappe._dict()).setdefault(d.deduction, 0.0)

		if currency == company_currency:
			ss_map[d.parent][d.deduction] += flt(d.amount)
		else:
			ss_map[d.parent][d.deduction] += flt(d.amount)
